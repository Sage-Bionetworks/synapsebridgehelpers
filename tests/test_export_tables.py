...
def test_manually_pass_source_tables_str(syn, tables, new_project, sample_table):
    source_table = tables["schema"][0]["id"]
    source_tables = {source_table: sample_table}
    exported_table = export_tables(
            syn,
            table_mapping = source_table,
...
def test_manually_pass_source_tables_list(syn, tables, new_project, sample_table):
    source_table = tables["schema"][0]["id"]
    source_table_2 = tables["schema"][1]["id"]
    source_tables = {source_table: sample_table, source_table_2: sample_table}
    exported_table = export_tables(
            syn,
            table_mapping = [s["id"] for s in tables["schema"]],
            source_tables = source_tables,
...
def test_manually_pass_source_tables_dict(syn, tables, new_project, sample_table):
    source_table = tables["schema"][0]["id"]
    schema = sc.Schema(
            name = tables["schema"][0]["name"],
            columns = tables["columns"][0],
